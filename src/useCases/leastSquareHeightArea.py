...
class LeastSquareHeightArea:
    ...
    def plot_least_squares(self, file_path):
        ...
        height_list = np.array(xlsx_dataFrame.stage_m)
        height_mtx = height_list.reshape(-1, 1)

        area_list = np.array(xlsx_dataFrame.area_sq_m)
        ...
        self.score = round(linear_model_height_mean_area.score(height_mtx, area_list), 4)
